...
# Import from root-level ai_model
from ai_model.rag_pipeline import LangChainRAG

# Import from backend utils
from backend.utils.ipfs_client import ipfs_client
from backend.utils.blockchain_client import blockchain_client
...
CONVERSATIONAL_RESPONSES = {
    "greetings": {
        ...
        "en": "Hello! I am KrishiMitr. How can I assist you?",
        "hi": "नमस्ते! मैं कृषि मित्र हूँ। मैं आपकी कैसे सहायता कर सकता हूँ?"
    }
}
...
def ask_question():
    if not rag_pipeline:
        return jsonify({"error": "Backend services unavailable."}), 500
    ...
    for intent, value in CONVERSATIONAL_RESPONSES.items():
        if any(keyword in query for keyword in value['keywords']):
            return jsonify({
                "answer": value.get(detected_lang, value['en']),
